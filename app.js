...
const argv = yargs(hideBin(process.argv))
    .command('copy', 'Copy files', {
        ...
        'file-limit': { description: 'Limit number of files to operate', alias: 'l', type: 'number', default: 1000, requiresArg: true}
    }, async(argv) => {
       await handleCopyFiles(argv.source, argv.destination, argv['file-types'], argv.recursive, argv['post-delete'], argv['preserve-path'], argv['file-limit']);
    })
    .command('cut', 'Cut files', {
        ...
        await handleCutFiles(argv.source, argv.destination, argv['file-types'], argv.recursive, argv['preserve-path'], argv['file-limit']);
        process.exit(0);
    })
